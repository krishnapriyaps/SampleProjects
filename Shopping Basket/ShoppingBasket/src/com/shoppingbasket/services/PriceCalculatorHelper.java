...
public class PriceCalculatorHelper {
	...
	/**
	 * Method check whether purchase order of item meets quantity requirement for
	 * offer
	 * 
	 * @param Offer             object
	 * @param ItemPurchaseorder
	 * 
	 * @return Return boolean true if offer is valid Return boolean false if offer
	 *         is not valid
	 */
	 ...
	 * @param ItemPurchaseorder
	 * 
	 * @return Positive Integer greater than zero indicate the limit on number of
	 *         items discount is applicable on. Return value zero indicate offer
	...
	public int findQuantityLimit(Offer offer, ItemPurchaseOrders itemPurchaseOrders, String discountedItemName) {
		...
		if (offer.getOfferOnItemQuantity() > 0) {
			logger.debug("Offer details, Discount of " + offer.getDiscountPercentage() + "% on " + discountedItemName
					+ " upto " + offer.getDiscountedItemQuantity() + " purchases");
